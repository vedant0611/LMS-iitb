...
# we'll create a function that takes a plain-text password as input and returns its corresponding hash value using bcrypt
def get_password_hash(password):
    return pwd_context.hash(password)

# When a user tries to log in, we need to verify whether the provided password matches the stored hashed password. We'll use the verify() method provided by bcrypt to do this.
